...
try {
    ...
    const credentials = JSON.parse(GCS_CREDENTIALS_JSON_STRING);
    ...
    // Set storage ke null jika ada error inisialisasi agar penanganan error di POST lebih mudah
    storage = null;
}
...
export async function POST(request) {
    ...
    try {
        ...
        if (!name || !mimeType || !data || !userId || !transactionId) {
            return NextResponse.json({ error: 'Data file tidak lengkap (nama, mimeType, data, userId, atau transactionId hilang).' }, { status: 400 });
        }
        ...
        if (error.code === 403) {
            errorMessage = 'Izin GCS ditolak. Pastikan Service Account memiliki peran "Storage Object Creator" dan "Storage Object Viewer" di bucket.';
        } else if (error.message.includes('bucket')) {
            errorMessage = 'Nama bucket GCS tidak valid atau tidak ada.';
        } else if (error.message.includes('JSON') || error.message.includes('credentials')) { // Tangkap error kredensial lebih luas
