// components/dashboard/HelpDeskSection.js
"use client";
...
export default function HelpDeskSection({ 
  ...
  // Fungsi helper untuk menentukan style status tiket
  ...
  return (
      ...
      <div className="space-y-4">
        ...
        <form className="space-y-4" onSubmit={handleHelpDeskSubmit}>
          <div>
            ...
            <textarea 
              id="question" 
              name="question" // Menambahkan atribut name untuk kemudahan akses di handler
              rows="4" 
              required 
          ...
          <button
            type="submit"
            className="w-full bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700 text-sm font-medium flex items-center justify-center disabled:opacity-50"
            disabled={helpDeskFormLoading}
          >
            {helpDeskFormLoading ? <SmallSpinner /> : 'Kirim Pertanyaan'}
          </button>
        </form>
        ...
        <div className="mt-8">
          ...
          {loadingHelpDeskTickets ? <ListSkeleton /> : (
            <div className="max-h-60 overflow-y-auto pr-2">
              ...
              {Array.isArray(userHelpDeskTickets) && userHelpDeskTickets.length > 0 ? (
                userHelpDeskTickets.map((ticket) => (
                  <div key={ticket.TicketId} className="flex flex-col py-2 border-b border-gray-200 last:border-b-0">
                    ...
                    <div className="flex justify-between items-center text-xs text-gray-500 mt-1">
                      ...
                      <span className={`px-2 py-0.5 rounded-full font-semibold ${getStatusClasses(ticket.Status)}`}>
                        {ticket.Status}
                      </span>
                    </div>
    ...
    </div> // Tag penutup yang berlebihan sudah dihapus
  );
}